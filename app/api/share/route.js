...
export async function POST(request) {
  try {
    ...
    if (process.env.BLOB_READ_WRITE_TOKEN) {
      ...
      // ── Running in production but missing token ─────────────────
      console.error('[api/share] BLOB_READ_WRITE_TOKEN is missing in production environment.');
      return Response.json({ error: 'Blob storage is not configured. Add BLOB_READ_WRITE_TOKEN to environment variables.' }, { status: 500 });
    } else {
