 ...
 * - zoom: initial zoom level to frame the face nicely
 *
 ...
 * @param {number} targetAspect - Aspect ratio of the target photo region (w/h)
 * @returns {Promise<{ centerX: number, centerY: number, zoom: number }>}
 */
export async function getInitialCrop(imageUrl, imgWidth, imgHeight, targetAspect) {
  const face = await detectFace(imageUrl, imgWidth, imgHeight);

  if (face) {
    // Center on the face, with some padding
    const padding = 1.6; // Show 60% more around the face
    const faceCenterX = face.x + face.width / 2;
    const faceCenterY = face.y + face.height / 2;

    // Calculate zoom so the face fills roughly 40% of the crop region
    const faceSize = Math.max(face.width, face.height) * padding;
    const cropWidth = targetAspect >= 1 ? faceSize : faceSize * targetAspect;
    const cropHeight = targetAspect >= 1 ? faceSize / targetAspect : faceSize;
    const zoom = Math.min(
      imgWidth / cropWidth,
